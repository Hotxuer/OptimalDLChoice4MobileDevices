...
public class MultiBoxTracker {
  ...
  public List<TrackedRecognition> trackedObjects = new LinkedList<TrackedRecognition>();
  ...
  private Matrix frameToFrameMatrix;
  ...
  private Matrix getFrameToFrameMatrix() { return  frameToFrameMatrix; }
  ...
  // 在canvas上画框和标签
  // 需要的参数有（都在此类中）：sensorOrientation、trackedObjects、objectTracker？为null也可以
  public synchronized void draw(final Canvas canvas) {
    ...
    for (TrackedRecognition recognition : trackedObjects) {
      ...
      if (Store.isWhetherLocal()) {
        ...
        getFrameToFrameMatrix().mapRect(trackedPos);
      }
      boxPaint.setColor(recognition.color);

      final float cornerSize = Math.min(trackedPos.width(), trackedPos.height()) / 8.0f;
      canvas.drawRoundRect(trackedPos, cornerSize, cornerSize, boxPaint);
      logger.i("当前画框："+trackedPos.left+", "+trackedPos.top+", "+trackedPos.right+", "+trackedPos.bottom+", "+recognition.color+", "+sensorOrientation);
  ...
  private void processResults(
    ...
    for (final Recognition result : results) {
      ...
      // 将object所在的Rect从frame版本转换到screen版本
      ...
      // 不满足大小要求的要略去，通过的放入待识别的rectsToTrack中
    ...
    // 在这里才开始对每个通过检测的object（rect）进行（可能的）识别
  ...
  private void handleDetection(
    ...
    // 这个trackedObjects是已经检验完成的object，如果当前物体通过也会放进去

    // 看看是不是跟检测出来的物体重合太多
    // 后面一系列的都是对检测出来的物体做的筛选
  ...
  public void setTrackedObjects(List<TrackedRecognition> trackedRecognitions) {
    trackedObjects.clear();
